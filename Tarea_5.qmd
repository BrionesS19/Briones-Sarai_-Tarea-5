...
dderf = lambda x: 4*np.exp(2*x)+4*np.cos(2x)
...
b)  $f(x)=log(x+2)-(x+1)^2$, $x\in [0,5]$ Realiza la respectiva gráfica en el intervalo dado.
...
c)  $f(x)=\sqrt{x} sen(x^2)$, $x\in [0,\pi]$ Realiza la respectiva gráfica en el intervalo dado.
...
d)  $f(x)=(cos\,3x)^2-e^{2x}$, $x\in [0,\pi/2]$ Realiza la respectiva gráfica en el intervalo dado.
...
\# Ejericicio 2 Aproximar las siguientes integrales, cada caso hacer una gráfica de la función e indicar el área bajo la curva.
...
plt.plot(x, pdf_vals, label='PDF - Normal estándar')
plt.fill_between(x, pdf_vals, where=(x > 0) & (x < 1.5), color='skyblue', alpha=0.5, label='Área: P(0 < X < 1.5)')
...
# probabilidad mediante integración 
prob_integral, _ = quad(lambda x: norm.pdf(x, loc=mu, scale=sigma), 0, 1.5)

# probabilidad con la función CDF
prob_cdf = norm.cdf(1.5, loc=mu, scale=sigma) - norm.cdf(0, loc=mu, scale=sigma)

print(f"Probabilidad calculada por integración: {prob_integral:.6f}")
print(f"Probabilidad calculada por CDF:         {prob_cdf:.6f}")
print(f"Diferencia absoluta:                    {abs(prob_integral - prob_cdf):.2e}")
